...
// This will remove the errors of undefined TC_BBSRAM_SIZE when logger module is activated.
// Fixme: using ??
...
#define TC_BUS_OFFSET       1                  /* RP2040 buses are 0 based and adjustment is needed */
#define tc_spibus_initialize(bus_num_1based)   rp2040_spibus_initialize(TC_BUS_NUMBER_FROM_TC(bus_num_1based))

#define tc_i2cbus_initialize(bus_num_1based)   rp2040_i2cbus_initialize(TC_BUS_NUMBER_FROM_TC(bus_num_1based))
#define tc_i2cbus_uninitialize(pdev)           rp2040_i2cbus_uninitialize(pdev)
...
#define tc_arch_configgpio(pinset)		rp2040_gpioconfig(pinset)			// Defined in io_pins/rp2040_pinset.c
#define tc_arch_unconfiggpio(pinset)           rp2040_gpio_init(pinset & GPIO_NUM_MASK)	// Reset the pin as input SIO
#define tc_arch_gpioread(pinset)               rp2040_gpio_get(pinset & GPIO_NUM_MASK)		// Use gpio_get
#define tc_arch_gpiowrite(pinset, value)       rp2040_gpio_put(pinset & GPIO_NUM_MASK, value)	// Use gpio_put
#define tc_arch_gpiosetevent(pinset,r,f,e,fp,a) rp2040_setgpioevent(pinset,r,f,e,fp,a)		// Defined in io_pins/rp2040_pinset.c
...
#define TC_MAKE_GPIO_INPUT(gpio) (gpio | GPIO_PU | GPIO_FUN(RP2040_GPIO_FUNC_SIO))
#define TC_MAKE_GPIO_OUTPUT_CLEAR(gpio) (gpio | GPIO_OUT | GPIO_FUN(RP2040_GPIO_FUNC_SIO))
#define TC_MAKE_GPIO_OUTPUT_SET(gpio) (gpio | GPIO_OUT | GPIO_SET | GPIO_FUN(RP2040_GPIO_FUNC_SIO))

#define TC_GPIO_PIN_OFF(pinset) ((pinset & GPIO_NUM_MASK) | GPIO_FUN(RP2040_GPIO_FUNC_SIO) | GPIO_PD)
