...
static bool board_test_force_pin(void)
{
	...
	for (volatile unsigned cycles = 0; cycles < 10; cycles++) {
		tc_arch_gpiowrite(BOARD_FORCE_BL_PIN_OUT, 1);

		for (unsigned count = 0; count < 20; count++) {
			if (tc_arch_gpioread(BOARD_FORCE_BL_PIN_IN) != 0) {
				vote++;
			}
		...
		tc_arch_gpiowrite(BOARD_FORCE_BL_PIN_OUT, 0);
	...
	for (samples = 0; samples < 200; samples++) {
		if ((tc_arch_gpioread(BOARD_FORCE_BL_PIN) ? 1 : 0) == BOARD_FORCE_BL_STATE) {
			vote++;
		}
	}
...
board_init(void)
{
	...
	tc_arch_configgpio(BOARD_POWER_PIN_OUT);
	tc_arch_gpiowrite(BOARD_POWER_PIN_OUT, BOARD_POWER_ON);
#endif
...
#  if defined(USE_VBUS_PULL_DOWN)
	tc_arch_configgpio((GPIO_OTGFS_VBUS & GPIO_PUPD_MASK) | GPIO_PULLDOWN);
#  else
	tc_arch_configgpio((GPIO_OTGFS_VBUS & GPIO_PUPD_MASK) | GPIO_FLOAT);
#  endif
#endif
#endif
	...
	tc_arch_configgpio(BOARD_FORCE_BL_PIN_IN);
	tc_arch_configgpio(BOARD_FORCE_BL_PIN_OUT);
#endif

#if defined(BOARD_FORCE_BL_PIN)
	/* configure the force BL pins */
	tc_arch_configgpio(BOARD_FORCE_BL_PIN);
#endif

#if defined(BOARD_PIN_LED_ACTIVITY)
	/* Initialize LEDs */
	tc_arch_configgpio(BOARD_PIN_LED_ACTIVITY);
#endif
#if defined(BOARD_PIN_LED_BOOTLOADER)
	/* Initialize LEDs */
	tc_arch_configgpio(BOARD_PIN_LED_BOOTLOADER);
#endif
}
...
board_deinit(void)
{
...
#if INTERFACE_USB
	tc_arch_configgpio(MK_GPIO_INPUT(GPIO_OTGFS_VBUS));
	putreg32(RCC_AHB1RSTR_OTGFSRST, STM32_RCC_AHB1RSTR);
#endif

#if defined(BOARD_FORCE_BL_PIN_IN) && defined(BOARD_FORCE_BL_PIN_OUT)
	/* deinitialise the force BL pins */
	tc_arch_configgpio(MK_GPIO_INPUT(BOARD_FORCE_BL_PIN_IN));
	tc_arch_configgpio(MK_GPIO_INPUT(BOARD_FORCE_BL_PIN_OUT));
#endif

#if defined(BOARD_FORCE_BL_PIN)
	/* deinitialise the force BL pin */
	tc_arch_configgpio(MK_GPIO_INPUT(BOARD_FORCE_BL_PIN));
#endif
	 ...
	 */
	tc_arch_configgpio(MK_GPIO_INPUT(BOARD_POWER_PIN_OUT));
#endif

#if defined(BOARD_PIN_LED_ACTIVITY)
	/* Initialize LEDs */
	tc_arch_configgpio(MK_GPIO_INPUT(BOARD_PIN_LED_ACTIVITY));
#endif
#if defined(BOARD_PIN_LED_BOOTLOADER)
	/* Initialize LEDs */
	tc_arch_configgpio(MK_GPIO_INPUT(BOARD_PIN_LED_BOOTLOADER));
#endif
...
led_on(unsigned led)
{
	...
	case LED_ACTIVITY:
#if defined(BOARD_PIN_LED_ACTIVITY)
		tc_arch_gpiowrite(BOARD_PIN_LED_ACTIVITY, BOARD_LED_ON);
#endif
		break;

	case LED_BOOTLOADER:
#if defined(BOARD_PIN_LED_BOOTLOADER)
		tc_arch_gpiowrite(BOARD_PIN_LED_BOOTLOADER, BOARD_LED_ON);
#endif
		break;
	}
}
...
led_off(unsigned led)
{
	...
	case LED_ACTIVITY:
#if defined(BOARD_PIN_LED_ACTIVITY)
		tc_arch_gpiowrite(BOARD_PIN_LED_ACTIVITY, BOARD_LED_OFF);
#endif
		break;

	case LED_BOOTLOADER:
#if defined(BOARD_PIN_LED_BOOTLOADER)
		tc_arch_gpiowrite(BOARD_PIN_LED_BOOTLOADER, BOARD_LED_OFF);
#endif
		break;
	}
}
...
led_toggle(unsigned led)
{
	...
	case LED_ACTIVITY:
#if defined(BOARD_PIN_LED_ACTIVITY)
		tc_arch_gpiowrite(BOARD_PIN_LED_ACTIVITY, tc_arch_gpioread(BOARD_PIN_LED_ACTIVITY) ^ 1);
#endif
		break;

	case LED_BOOTLOADER:
#if defined(BOARD_PIN_LED_BOOTLOADER)
		tc_arch_gpiowrite(BOARD_PIN_LED_BOOTLOADER, tc_arch_gpioread(BOARD_PIN_LED_BOOTLOADER) ^ 1);
#endif
		break;
	}
}
...
bootloader_main(void)
{
	...
	if (tc_arch_gpioread(BOARD_VBUS) != 0) {
		usb_connected = true;
