...
foreach(test_name ${tests})
	...
	configure_file(${TC_SOURCE_DIR}/posix-configs/SITL/init/test/test_template.in ${TC_SOURCE_DIR}/posix-configs/SITL/init/test/test_${test_name}_generated)

	add_test(NAME ${test_name_prefix}
		COMMAND $<TARGET_FILE:tc>
			-s ${TC_SOURCE_DIR}/posix-configs/SITL/init/test/test_${test_name}_generated
			-t ${TC_SOURCE_DIR}/test_data
			${TC_SOURCE_DIR}/ROMFS/tcfmu_test
...
foreach(test_name ${cmd_tests})
	set(test_name_prefix sitl-${test_name})
	configure_file(${TC_SOURCE_DIR}/posix-configs/SITL/init/test/test_cmd_template.in ${TC_SOURCE_DIR}/posix-configs/SITL/init/test/test_${test_name}_generated)
...
add_test(NAME sitl-mavlink
	COMMAND $<TARGET_FILE:tc>
		-s ${TC_SOURCE_DIR}/posix-configs/SITL/init/test/test_mavlink
		-t ${TC_SOURCE_DIR}/test_data
		${TC_SOURCE_DIR}/ROMFS/tcfmu_test
	WORKING_DIRECTORY ${SITL_WORKING_DIR}
)
...
add_test(NAME sitl-imu_filtering
	COMMAND $<TARGET_FILE:tc>
		-s ${TC_SOURCE_DIR}/posix-configs/SITL/init/test/test_imu_filtering
		-t ${TC_SOURCE_DIR}/test_data
		${TC_SOURCE_DIR}/ROMFS/tcfmu_test
	WORKING_DIRECTORY ${SITL_WORKING_DIR}
)
...
# 	COMMAND $<TARGET_FILE:tc>
# 		-s ${TC_SOURCE_DIR}/posix-configs/SITL/init/test/test_shutdown
# 		-t ${TC_SOURCE_DIR}/test_data
# 		${TC_SOURCE_DIR}/ROMFS/tcfmu_test
# 	WORKING_DIRECTORY ${SITL_WORKING_DIR}
# )
...
add_test(NAME dyn
	COMMAND $<TARGET_FILE:tc> -s "${TC_SOURCE_DIR}/posix-configs/SITL/init/test/test_dyn_hello"
	WORKING_DIRECTORY ${TC_BINARY_DIR}/src/examples/dyn_hello
)
...
foreach(cmd_name ${test_cmds})
	configure_file(${TC_SOURCE_DIR}/posix-configs/SITL/init/test/cmd_template.in ${TC_SOURCE_DIR}/posix-configs/SITL/init/test/cmd_${cmd_name}_generated)

	add_test(NAME posix_${cmd_name}
		COMMAND $<TARGET_FILE:tc>
			${TC_SOURCE_DIR}/ROMFS/tcfmu_test
			-s ${TC_SOURCE_DIR}/posix-configs/SITL/init/test/cmd_${cmd_name}_generated
			-t ${TC_SOURCE_DIR}/test_data
		WORKING_DIRECTORY ${SITL_WORKING_DIR}
	)
...
if(CMAKE_BUILD_TYPE STREQUAL Coverage)
	...
	# TODO:
	#setup_target_for_coverage(mavsdk_coverage "${TC_SOURCE_DIR}/test/mavsdk_tests/mavsdk_test_runner.py --speed-factor 20 --iterations 1 --fail-early" mavsdk)
endif()
