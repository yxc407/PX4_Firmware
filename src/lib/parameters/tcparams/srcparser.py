...
class SourceParser(object):
    ...
    re_tc_parameter_definition = re.compile(r'TC_PARAM_DEFINE_([A-Z_][A-Z0-9_]*)\s*\(([A-Z_][A-Z0-9_]*)\s*\)\s*;')
    re_tc_param_default_definition = re.compile(r'#define\s*PARAM_([A-Z_][A-Z0-9_]*)\s*([^ ,\)]+)\s*')
    ...
    def Parse(self, contents):
        ...
        for line in self.re_split_lines.split(contents):
            ...
            if self.re_comment_start.match(line):
                ...
                m = self.re_tc_param_default_definition.match(line)
                ...
                if m:
                    ...
                    m = self.re_tc_parameter_definition.match(line)
                    if m:
                        tp, name = m.group(1, 2)
