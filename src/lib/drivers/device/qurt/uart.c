...
int qurt_uart_open(const char *dev, speed_t speed)
{
	if (_callbacks_configured) {
		...
		if ((port_number == 0) && (endptr == dev)) {
			TC_ERR("Could not convert %s into a valid uart port number", dev);
			return -1;
		}
		...
		TC_ERR("Cannot open uart until callbacks have been configured");
	}
...
{
	if (fd < 0) {
		TC_ERR("invalid fd %d for %s", fd, __FUNCTION__);
		return -1;
	}

	if (buf == NULL) {
		TC_ERR("NULL buffer pointer in %s", fd, __FUNCTION__);
		return -1;
	}

	if (len == 0) {
		TC_ERR("Zero length buffer in %s", __FUNCTION__);
		return -1;
	}
	...
	if (_callbacks_configured) {
		...
		TC_ERR("Cannot write to uart until callbacks have been configured");
	}
...
int qurt_uart_read(int fd, char *buf, size_t len, uint32_t timeout_us)
{
	...
	if (_callbacks_configured) {
		...
		// TC_INFO("UART interval counter = %d", interval_counter);
		int read_len = 0;
		...
		do {
			...
			interval_counter--;
			tc_usleep(UART_READ_POLL_INTERVAL_US);
		} while (interval_counter);

		// if (read_len <= 0) TC_INFO("Warning, UART read timed out");
		// else TC_INFO("UART read %d bytes, counter = %d", read_len, interval_counter);
		return read_len;

	} else {
		TC_ERR("Cannot read from uart until callbacks have been configured");
	}
