...
if validate:
    ...
    if m:
        ...
        if major < 1 or (major == 1 and minor < 9):
            print("")
            print("Error: TC version too low, expected at least v1.9.0")
        ...
        print("")
        print("The expected format is 'v<TC version>[-<custom version>]'")
        print("  <TC version>: v<major>.<minor>.<patch>[-rc<rc>|-beta<beta>|-alpha<alpha>|-dev]")
        ...
        print("See also https://docs.tc.io/main/en/dev_setup/building_tc.html#building-for-nuttx")
        print("")
        sys.exit(1)
...
header += f"""
#define TC_GIT_VERSION_STR "{git_version}"
#define TC_GIT_VERSION_BINARY 0x{git_version_short}
#define TC_GIT_TAG_STR "{git_tag}"
#define TC_GIT_BRANCH_NAME "{git_branch_name}"

#define TC_GIT_OEM_VERSION_STR  "{oem_tag}"

#define TC_GIT_TAG_OR_BRANCH_NAME "{tag_or_branch}" // special variable: git tag, release or master branch
"""
