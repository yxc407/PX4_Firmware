...
bool RCTest::crsfTest()
{
	...
	ut_test(fp);
	//TC_INFO("loading data from: %s", filepath);
	...
	while (fgets(line, line_size, fp) != nullptr)  {
		...
		if (strncmp(line, "INPUT ", 6) == 0) {
			...
			if (has_decoded_values) {
				TC_ERR("Parser decoded values that are not in the test file (line=%i)", line_counter);
				return false;
			}
			...
			if (!has_decoded_values) {
				TC_ERR("Test file contains decoded values but the parser did not decode anything (line=%i)", line_counter);
				return false;
			}
			...
			while (sscanf(file_buffer, "%x, %n", &expected_rc_value, &offset) > 0) {
				...
				if (abs(expected_rc_value - (int)rc_values[expected_num_channels]) > 10) {
					TC_ERR("File line: %i, channel: %i", line_counter, expected_num_channels);
			...
			if (expected_num_channels != num_values) {
				TC_ERR("File line: %d", line_counter);
...
{
	return dsmTest(TEST_DATA_PATH "dsm_x_dx9_tc_binding_data.txt", 16, 1, 1499);
}
...
bool RCTest::dsmTest(const char *filepath, unsigned expected_chancount, unsigned expected_dropcount, unsigned chan0)
{
	...
	while (EOF != (ret = fscanf(fp, "%f,%x,,", &f, &x))) {
		...
		if (result) {
			...
			//TC_INFO("decoded packet with %d channels and %s encoding:", num_values, (dsm_11_bit) ? "11 bit" : "10 bit");

			for (unsigned i = 0; i < num_values; i++) {
				//TC_INFO("chan #%u:\t%d", i, (int)rc_values[i]);
			}
		}

		if (last_drop != (dsm_frame_drops)) {
			//TC_INFO("frame dropped, now #%d", (dsm_frame_drops));
...
bool RCTest::sbus2Test()
{
	...
	while (EOF != (ret = fscanf(fp, "%f,%x,,", &f, &x))) {
		...
		if (result) {
			//TC_INFO("decoded packet");
		}

		// }

		if (last_drop != (sbus_frame_drops + sbus_frame_resets)) {
			TC_WARN("frame dropped, now #%d", (sbus_frame_drops + sbus_frame_resets));
...
bool RCTest::st24Test()
{
	...
	while (EOF != (ret = fscanf(fp, "%f,%x,,", &f, &x))) {
		...
		if (!st24_decode(b, &rssi, &rx_count, &channel_count, channels, sizeof(channels) / sizeof(channels[0]))) {
			//TC_INFO("decoded: %u channels (converted to PPM range)", (unsigned)channel_count);

			for (unsigned i = 0; i < channel_count; i++) {
				//int16_t val = channels[i];
				//TC_INFO("channel %u: %d 0x%03X", i, static_cast<int>(val), static_cast<int>(val));
			}
		}
	}
