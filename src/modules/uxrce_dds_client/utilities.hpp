			       ...
			       uxrObjectId &datawriter_id)
{
	...
	if (!uxr_run_session_until_all_status(session, 1000, requests, status, 3)) {
		TC_ERR("create entities failed: %s, topic: %i publisher: %i datawriter: %i",
			topic_name, status[0], status[1], status[2]);
		return false;

	} else {
		TC_INFO("successfully created %s data writer, topic id: %d", topic_name, topic_id.id);
	}
			       ...
			       const char *type_name, uint16_t queue_depth)
{
	...
	if (!uxr_run_session_until_all_status(session, 1000, requests, status, 3)) {
		TC_ERR("create entities failed: %s %i %i %i", topic_name,
