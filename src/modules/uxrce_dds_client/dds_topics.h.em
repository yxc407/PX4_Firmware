		...
		if (send_subscriptions[idx].subscription.update(&topic_data)) {
			...
			if (send_subscriptions[idx].data_writer.id != UXR_INVALID_ID) {
				...
				if (uxr_prepare_output_stream(session, best_effort_stream_id, send_subscriptions[idx].data_writer, &ub, topic_size) != UXR_INVALID_REQUEST_ID) {
					...
					//TC_ERR("Error uxr_prepare_output_stream UXR_INVALID_REQUEST_ID %s", send_subscriptions[idx].subscription.get_topic()->o_name);
				}

			} else {
				//TC_ERR("Error UXR_INVALID_ID %s", send_subscriptions[idx].subscription.get_topic()->o_name);
			}
...
@[    end for]@
	...
	default:
		TC_ERR("unknown object id: %i", object_id.id);
		break;
	}
}
