...
{
	tc_arch_configgpio(BROADCOM_AFBR_S50_S2PI_CS);

	s2pi_.spidev = tc_spibus_initialize(BROADCOM_AFBR_S50_S2PI_SPI_BUS);

	tc_arch_configgpio(BROADCOM_AFBR_S50_S2PI_IRQ);
	tc_arch_gpiosetevent(BROADCOM_AFBR_S50_S2PI_IRQ, false, true, false, &gpio_falling_edge, NULL);
...
status_t S2PI_CaptureGpioControl(void)
{
	...
	tc_arch_configgpio(TC_MAKE_GPIO_OUTPUT_SET(s2pi_.GPIOs[S2PI_CLK]));
	tc_arch_configgpio(TC_MAKE_GPIO_INPUT(s2pi_.GPIOs[S2PI_MISO]));
	tc_arch_configgpio(TC_MAKE_GPIO_OUTPUT_SET(s2pi_.GPIOs[S2PI_MOSI]));
...
status_t S2PI_WriteGpioPin(s2pi_slave_t slave, s2pi_pin_t pin, uint32_t value)
{
	...
	tc_arch_gpiowrite(s2pi_.GPIOs[pin], value);
...
status_t S2PI_ReadGpioPin(s2pi_slave_t slave, s2pi_pin_t pin, uint32_t *value)
{
	...
	*value = tc_arch_gpioread(s2pi_.GPIOs[pin]);
...
status_t S2PI_CycleCsPin(s2pi_slave_t slave)
{
	...
	tc_arch_gpiowrite(s2pi_.GPIOs[S2PI_CS], 0);
	tc_arch_gpiowrite(s2pi_.GPIOs[S2PI_CS], 1);
...
{
	return tc_arch_gpioread(s2pi_.GPIOs[S2PI_IRQ]);
}
