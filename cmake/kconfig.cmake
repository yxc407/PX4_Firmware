set(BOARD_DEFCONFIG ${TC_CONFIG_FILE} CACHE FILEPATH "path to defconfig" FORCE)
set(BOARD_CONFIG ${TC_BINARY_DIR}/boardconfig CACHE FILEPATH "path to config" FORCE)
...
set(COMMON_KCONFIG_ENV_SETTINGS
	...
	VENDOR=${TC_BOARD_VENDOR}
	MODEL=${TC_BOARD_MODEL}
	LABEL=${TC_BOARD_LABEL}
...
if(EXISTS ${BOARD_DEFCONFIG})
	...
	if(${LABEL} MATCHES "default" OR ${LABEL} MATCHES "recovery" OR ${LABEL} MATCHES "bootloader" OR ${LABEL} MATCHES "canbootloader")
		...
		# Generate boardconfig from default.tcboard and {label}.tcboard
		execute_process(
			COMMAND ${CMAKE_COMMAND} -E env ${COMMON_KCONFIG_ENV_SETTINGS}
			${PYTHON_EXECUTABLE} ${TC_SOURCE_DIR}/Tools/kconfig/merge_config.py Kconfig ${BOARD_CONFIG} ${tc_BOARD_DIR}/default.tcboard ${BOARD_DEFCONFIG}
			WORKING_DIRECTORY ${TC_SOURCE_DIR}
			OUTPUT_VARIABLE DUMMY_RESULTS
		)
	endif()
    ...
    execute_process(
	COMMAND ${CMAKE_COMMAND} -E env ${COMMON_KCONFIG_ENV_SETTINGS}
		${GENCONFIG_PATH} --header-path ${TC_BINARY_DIR}/tc_boardconfig.h
	WORKING_DIRECTORY ${TC_SOURCE_DIR}
	OUTPUT_VARIABLE DUMMY_RESULTS
	)
	...
	foreach(NameAndValue ${ConfigContents})
		...
		if(Drivers)
			...
			# Trick circumvent TC src naming problem with underscores and slashes
			if(EXISTS ${TC_SOURCE_DIR}/src/drivers/${driver})
				list(APPEND config_module_list drivers/${driver})
			elseif(EXISTS ${TC_SOURCE_DIR}/src/drivers/${driver_path})
				list(APPEND config_module_list drivers/${driver_path})
			elseif(EXISTS ${TC_SOURCE_DIR}/src/drivers/${driver_p3_folder}/${driver_p3_subfolder}/${driver_p3_subsubfolder})
				list(APPEND config_module_list drivers/${driver_p3_folder}/${driver_p3_subfolder}/${driver_p3_subsubfolder})
			elseif(EXISTS ${TC_SOURCE_DIR}/src/drivers/${driver_p1_folder}/${driver_p1_subfolder})
				list(APPEND config_module_list drivers/${driver_p1_folder}/${driver_p1_subfolder})
			elseif(EXISTS ${TC_SOURCE_DIR}/src/drivers/${driver_p4_folder}/${driver_p4_subfolder})
				list(APPEND config_module_list drivers/${driver_p4_folder}/${driver_p4_subfolder})
			elseif(EXISTS ${TC_SOURCE_DIR}/src/drivers/${driver_p2_folder}/${driver_p2_subfolder})
				list(APPEND config_module_list drivers/${driver_p2_folder}/${driver_p2_subfolder})
			elseif(EXISTS ${TC_SOURCE_DIR}/src/drivers/${driver_p5_folder}/${driver_p5_subfolder}/${driver_p5_subsubfolder})
		...
		if(Modules)
			...
			# Trick circumvent TC src naming problem with underscores and slashes
			if(EXISTS ${TC_SOURCE_DIR}/src/modules/${module})
				list(APPEND config_module_list modules/${module})
			elseif(EXISTS ${TC_SOURCE_DIR}/src/modules/${module_path})
				list(APPEND config_module_list modules/${module_path})
			elseif(EXISTS ${TC_SOURCE_DIR}/src/modules/${module_p1_folder}/${module_p1_subfolder})
				list(APPEND config_module_list modules/${module_p1_folder}/${module_p1_subfolder})
			elseif(EXISTS ${TC_SOURCE_DIR}/src/modules/${module_p2_folder}/${module_p2_subfolder})
	...
	if(PLATFORM)
		...
		set(TC_PLATFORM ${PLATFORM} CACHE STRING "tc board OS" FORCE)
		list(APPEND CMAKE_MODULE_PATH ${TC_SOURCE_DIR}/platforms/${TC_PLATFORM}/cmake)

		# platform-specific include path
		include_directories(${TC_SOURCE_DIR}/platforms/${TC_PLATFORM}/src/tc/common/include)
	endif()
	...
	if(NOT CONSTRAINED_FLASH AND NOT EXTERNAL_METADATA AND NOT ${TC_BOARD_LABEL} STREQUAL "test")
		list(APPEND romfs_extra_files
			${TC_BINARY_DIR}/parameters.json.xz
			${TC_BINARY_DIR}/events/all_events.json.xz
			${TC_BINARY_DIR}/actuators.json.xz
			)
	...
	list(APPEND romfs_extra_files ${TC_BINARY_DIR}/component_general.json.xz)
	...
	if(CONSTRAINED_FLASH)
		set(tc_constrained_flash_build "1" CACHE INTERNAL "constrained flash build" FORCE)
		add_definitions(-DCONSTRAINED_FLASH)
	endif()

	if(NO_HELP)
		add_definitions(-DCONSTRAINED_FLASH_NO_HELP="https://docs.tc.io/main/en/modules/modules_main.html")
	endif()

	if(CONSTRAINED_MEMORY)
		set(tc_constrained_memory_build "1" CACHE INTERNAL "constrained memory build" FORCE)
		add_definitions(-DCONSTRAINED_MEMORY)
	endif()

	if(TESTING)
		set(TC_TESTING "1" CACHE INTERNAL "testing enabled" FORCE)
	endif()

	if(ETHERNET)
		set(TC_ETHERNET "1" CACHE INTERNAL "ethernet enabled" FORCE)
	endif()

	if(CRYPTO)
		set(TC_CRYPTO "1" CACHE INTERNAL "tc crypto implementation" FORCE)
		add_definitions(-DTC_CRYPTO)
	endif()

	if(LINKER_PREFIX)
		set(TC_BOARD_LINKER_PREFIX ${LINKER_PREFIX} CACHE STRING "tc board linker prefix" FORCE)
	else()
		set(TC_BOARD_LINKER_PREFIX "" CACHE STRING "tc board linker prefix" FORCE)
	endif()
	...
	include(tc_impl_os)
	tc_os_prebuild_targets(OUT prebuild_targets BOARD ${TC_BOARD})

	# add board config directory src to build modules
	file(RELATIVE_PATH board_support_src_rel ${TC_SOURCE_DIR}/src ${TC_BOARD_DIR})
...
if(${LABEL} MATCHES "default" OR ${LABEL} MATCHES "bootloader" OR ${LABEL} MATCHES "canbootloader")
	add_custom_target(boardconfig
		...
		COMMAND ${CMAKE_COMMAND} -E remove ${TC_BINARY_DIR}/NuttX/apps_copy.stamp
		WORKING_DIRECTORY ${TC_SOURCE_DIR}
		USES_TERMINAL
	...
	add_custom_target(tc_savedefconfig
	...
	add_custom_target(boardconfig
		${CMAKE_COMMAND} -E env ${COMMON_KCONFIG_ENV_SETTINGS} ${MENUCONFIG_PATH} Kconfig
		COMMAND ${CMAKE_COMMAND} -E env ${COMMON_KCONFIG_ENV_SETTINGS} ${SAVEDEFCONFIG_PATH}
		COMMAND ${CMAKE_COMMAND} -E env ${COMMON_KCONFIG_ENV_SETTINGS} ${PYTHON_EXECUTABLE} ${TC_SOURCE_DIR}/Tools/kconfig/diffconfig.py -m ${TC_BOARD_DIR}/default.tcboard defconfig > ${BOARD_DEFCONFIG}
		COMMAND ${CMAKE_COMMAND} -E remove defconfig
		COMMAND ${CMAKE_COMMAND} -E remove ${TC_BINARY_DIR}/NuttX/apps_copy.stamp
		WORKING_DIRECTORY ${TC_SOURCE_DIR}
		USES_TERMINAL
