...
def is_everything_ready(config: Dict[str, str], build_dir: str) -> bool:
    ...
    if config['mode'] == 'sitl':
        if is_running('tc'):
            print("tc process already running\n"
                  "run `killall tc` and try again")
            result = False
        if not os.path.isfile(os.path.join(build_dir, 'bin/tc')):
            print("TC SITL is not built\n"
                  "run `DONT_RUN=1 "
                  "make tc_sitl gazebo mavsdk_tests` or "
                  "`DONT_RUN=1 make tc_sitl_default gazebo mavsdk_tests`")
            result = False
    ...
    def start_runners(self,
        ...
        if self.config['mode'] == 'sitl':
            if self.config['simulator'] == 'gazebo':
                ...
                # We must start the TC instance at the end, as starting
                # it in the beginning, then connecting Gazebo server freaks
                # out the TC (it needs to have data coming in when started),
                # and can lead to EKF to freak out, or the instance itself
                # to die unexpectedly.
                tc_runner = ph.Px4Runner(
                    os.getcwd(),
                    log_dir,
                    test['model'],
                    case,
                ...
                for env_key in test.get('env', []):
                    tc_runner.env[env_key] = str(test['env'][env_key])
                self.active_runners.append(tc_runner)
