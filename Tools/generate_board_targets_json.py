...
build_configs = []
excluded_boards = ['modalai_voxl2', 'tc_ros2']  # TODO: fix and enable
...
def process_target(tcboard_file, target_name):
    ret = None
    platform = None
    toolchain = None

    if tcboard_file.endswith("default.tcboard") or \
        tcboard_file.endswith("recovery.tcboard") or \
        tcboard_file.endswith("bootloader.tcboard"):
        kconf.load_config(tcboard_file, replace=True)
    else: # Merge config with default.tcboard
        default_kconfig = re.sub(r'[a-zA-Z\d_]+\.tcboard', 'default.tcboard', tcboard_file)
        kconf.load_config(default_kconfig, replace=True)
        kconf.load_config(tcboard_file, replace=False)
    ...
    assert platform, f"PLATFORM not found in {tcboard_file}"

    if platform not in excluded_platforms:
        # get the container based on the platform and toolchain
        if platform == 'posix':
            container = 'tcio/tc-dev-base-focal:2021-09-08'
            if toolchain:
                if toolchain.startswith('aarch64'):
                    container = 'tcio/tc-dev-aarch64:2022-08-12'
                elif toolchain == 'arm-linux-gnueabihf':
                    container = 'tcio/tc-dev-armhf:2023-06-26'
                else:
                    if verbose: print(f'unmatched toolchain: {toolchain}')
        elif platform == 'nuttx':
            container = 'tcio/tc-dev-nuttx-focal:2022-08-12'
        else:
...
for manufacturer in os.scandir(os.path.join(source_dir, 'boards')):
    ...
    for board in os.scandir(manufacturer.path):
        ...
        for files in os.scandir(board.path):
            if files.is_file() and files.name.endswith('.tcboard'):
