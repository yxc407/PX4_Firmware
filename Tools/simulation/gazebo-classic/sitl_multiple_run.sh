#!/bin/bash
# run multiple instances of the 'tc' binary, with the gazebo SITL simulation
# It assumes tc is already built, with 'make tc_sitl_default sitl_gazebo-classic'
...
world=${WORLD:=empty}
target=${TARGET:=tc_sitl_default}
vehicle_model=${VEHICLE_MODEL:="iris"}
export TC_SIM_MODEL=gazebo-classic_${vehicle_model}
...
if [ -z ${SCRIPT} ]; then
	...
	for target in ${SCRIPT}; do
		...
		while [ $m -lt ${target_number} ]; do
			export TC_SIM_MODEL=gazebo-classic_${target_vehicle}
