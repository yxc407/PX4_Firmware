...
export TC_SIM_MODEL=gazebo-classic_${model}
export TC_SIM_WORLD=${world}
...
if [ -x "$(command -v gazebo)" ]; then
	...
	if [ -z $TC_SITL_WORLD ]; then
		...
		if [ -f ${src_path}/Tools/simulation/gazebo-classic/sitl_gazebo-classic/worlds/${TC_SITL_WORLD}.world ]; then
			# Spawn world by name if exists in the worlds directory from environment variable
			world_path="${src_path}/Tools/simulation/gazebo-classic/sitl_gazebo-classic/worlds/${TC_SITL_WORLD}.world"
		else
			# Spawn world from environment variable with absolute path
			world_path="$TC_SITL_WORLD"
		fi
	fi
