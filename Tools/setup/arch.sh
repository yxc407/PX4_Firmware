...
## Bash script to setup TC development environment on Arch Linux.
...
echo
echo "Installing TC general dependencies"
...
if [[ $INSTALL_NUTTX == "true" ]]; then
	...
	# remove modem manager (interferes with TC serial port usage)
...
if [[ $INSTALL_SIM == "true" ]]; then
	echo
	echo "Installing TC simulation dependencies"
	...
	if [[ $INSTALL_GAZEBO == "true" ]]; then
		echo
		echo "Installing gazebo and dependencies for TC gazebo simulation"

		# TC gazebo simulation dependencies
		sudo pacman -S --noconfirm --needed \
			dmidecode \
			eigen \
			hdf5 \
