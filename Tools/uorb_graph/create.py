...
class Graph(object):
    ...
    def _extract_build_information(self, file_name, **kwargs):
        ...
        for line in datafile:
            if 'tc_add_module' in line: # must contain 'tc_add_module'
                found_module_def = True
            elif 'tc_add_library' in line: # must contain 'tc_add_module'
                tokens = line.split('(')
                if 1 < len(tokens):
                ...
                # ... but the hard-tabs *is* specified by TC coding standards, so it's likely to be consistent
...
class OutputJSON(object):
    ...
    def write(self, file_name):
        ...
        for topic in sorted(output_topics, key=len, reverse=True):
            ...
            node['url'] = 'https://github.com/TC/TC-Autopilot/blob/main/msg/'+topic_filename(topic)+'.msg'
            nodes.append(node)
